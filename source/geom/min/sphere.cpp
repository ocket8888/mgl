...
template <typename T, template <typename> class vec>
vec<T> min::sphere<T,vec>::closest_point(const vec<T> &p) const
{
...
template <typename T, template <typename> class vec>
const vec<T> &min::sphere<T,vec>::get_center() const
{
    return _center;
}

template <typename T, template <typename> class vec>
vec<T> min::sphere<T,vec>::get_extent() const
{
...
template <typename T, template <typename> class vec>
vec<T> min::sphere<T,vec>::get_min() const
{
    T factor = vec<T>::inverse_unit_length();
...
template <typename T, template <typename> class vec>
vec<T> min::sphere<T,vec>::get_max() const
{
    T factor = vec<T>::inverse_unit_length();
    return _center + vec<T>().set_all(_radius * factor);
}
...
template <typename T, template <typename> class vec>
T min::sphere<T,vec>::get_radius() const
{
    return _radius;
}

template <typename T, template <typename> class vec>
T min::sphere<T,vec>::get_square_radius() const
{
    return _radius2;
}

template <typename T, template <typename> class vec>
std::vector<std::pair<vec<T>, T>> min::sphere<T,vec>::grid(size_t scale) const
{
...
template <typename T, template <typename> class vec>
vec<T> min::sphere<T,vec>::normal(const vec<T> &p, T &length, const T tolerance) const
{
...
template <typename T, template <typename> class vec>
bool min::sphere<T,vec>::point_inside(const vec<T> &p) const
{
    vec<T> d = p - _center;
...
template <typename T, template <typename> class vec>
void min::sphere<T,vec>::set_position(const vec<T> &pos)
{
    _center = pos;
}

template <typename T, template <typename> class vec>
T min::sphere<T,vec>::square_distance(const vec<T> &p) const
{
...
template <typename T, template <typename> class vec>
T min::sphere<T,vec>::square_size() const
{
...
template <typename T, template <typename> class vec>
std::vector<std::pair<vec<T>, T>> min::sphere<T,vec>::subdivide() const
{
