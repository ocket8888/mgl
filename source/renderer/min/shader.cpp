...
void min::shader::print_errors() const
{
    GLint log_length = 0;
    if (glIsShader(_id))
    {
...
min::shader::shader(const std::string &path, const GLenum type) : _id(0), _type(type)
{
...
min::shader::shader(const min::mem_file &mem, const GLenum type) : _id(0), _type(type)
{
    // Check that all needed extensions are present
    check_extensions();
