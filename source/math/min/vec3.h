...
class vec3
{
  ...
  public:
    ...
    inline bool any_zero_outside(const vec3<T> &p, const vec3<T> &min, const vec3<T> &max) const
    {
    ...
    inline static std::tuple<int, T, T, int, T, T, int, T, T> grid_ray(const vec3<T> &extent, const vec3<T> &origin, const vec3<T> &dir, const vec3<T> &inv_dir)
    {
    ...
    template <typename F>
    inline static void grid_range(const vec3<T> &min, const vec3<T> &extent, const size_t scale,
                                  const vec3<T> &over_min, const vec3<T> &over_max,
                                  const F &f)
    {
    ...
    static inline bool project_sat(const coord_sys<T, min::vec3> &axis1, const vec3<T> &center1, const vec3<T> &extent1, const coord_sys<T, min::vec3> &axis2, const vec3<T> &center2, const vec3<T> &extent2)
    {
        ...
        const vec3<T> t = vec3<T>(d.dot(axis1.x()), d.dot(axis1.y()), d.dot(axis1.z()));
    ...
    static inline std::pair<vec3<T>, T> project_sat_penetration(
        const coord_sys<T, min::vec3> &axis1, const vec3<T> &center1, const vec3<T> &extent1,
        const coord_sys<T, min::vec3> &axis2, const vec3<T> &center2, const vec3<T> &extent2, const T tolerance)
    {
        ...
        if (index != -1)
        {
            ...
            const vec3<T> sign = (center1 - center2).sign();
    ...
    static inline std::pair<vec3<T>, T> project_sat_aligned_penetration(
        const vec3<T> &center1, const vec3<T> &extent1,
        const vec3<T> &center2, const vec3<T> &extent2, const T tolerance)
    {
    ...
    inline static std::vector<std::pair<vec3<T>, vec3<T>>> subdivide(const vec3<T> &min, const vec3<T> &max)
    {
        ...
        // Octant 0
        const vec3<T> min0 = vec3<T>(cx_hx, cy_hy, cz_hz);
        const vec3<T> max0 = vec3<T>(c.x(), c.y(), c.z());

        // Octant 1
        const vec3<T> min1 = vec3<T>(cx_hx, cy_hy, c.z());
        const vec3<T> max1 = vec3<T>(c.x(), c.y(), czhz);

        // Octant 2
        const vec3<T> min2 = vec3<T>(cx_hx, c.y(), cz_hz);
        const vec3<T> max2 = vec3<T>(c.x(), cyhy, c.z());

        // Octant 3
        const vec3<T> min3 = vec3<T>(cx_hx, c.y(), c.z());
        const vec3<T> max3 = vec3<T>(c.x(), cyhy, czhz);

        // Octant 4
        const vec3<T> min4 = vec3<T>(c.x(), cy_hy, cz_hz);
        const vec3<T> max4 = vec3<T>(cxhx, c.y(), c.z());

        // Octant 5
        const vec3<T> min5 = vec3<T>(c.x(), cy_hy, c.z());
        const vec3<T> max5 = vec3<T>(cxhx, c.y(), czhz);

        // Octant 6
        const vec3<T> min6 = vec3<T>(c.x(), c.y(), cz_hz);
        const vec3<T> max6 = vec3<T>(cxhx, cyhy, c.z());

        // Octant 7
        const vec3<T> min7 = vec3<T>(c.x(), c.y(), c.z());
        const vec3<T> max7 = vec3<T>(cxhx, cyhy, czhz);
    ...
    inline static std::vector<std::pair<vec3<T>, T>> subdivide_center(const vec3<T> &min, const vec3<T> &max, const T size)
    {
        ...
        // Octant 0
        const vec3<T> c0 = vec3<T>(cx_hx, cy_hy, cz_hz);

        // Octant 1
        const vec3<T> c1 = vec3<T>(cx_hx, cy_hy, czhz);

        // Octant 2
        const vec3<T> c2 = vec3<T>(cx_hx, cyhy, cz_hz);

        // Octant 3
        const vec3<T> c3 = vec3<T>(cx_hx, cyhy, czhz);

        // Octant 4
        const vec3<T> c4 = vec3<T>(cxhx, cy_hy, cz_hz);

        // Octant 5
        const vec3<T> c5 = vec3<T>(cxhx, cy_hy, czhz);

        // Octant 6
        const vec3<T> c6 = vec3<T>(cxhx, cyhy, cz_hz);
    ...
    inline static void sub_overlap(std::vector<uint_fast8_t> &out, const vec3<T> &min, const vec3<T> &max, const vec3<T> &center)
    {
        // Reserve space for output
        out.clear();
        out.reserve(8);
