...
template class min::quat<double>;

template <typename T>
min::quat<T> min::quat<T>::operator+(const min::quat<T> &q) const
{
    const T w = _w + q.w();
    const T x = _x + q.x();
    const T y = _y + q.y();
...
template <typename T>
min::quat<T> min::quat<T>::operator*(const T a) const
{
    const T w = _w * a;
    const T x = _x * a;
    const T y = _y * a;
...
template <typename T>
min::quat<T> &min::quat<T>::operator-=(const min::quat<T> &q)
{
    _w -= q.w();
    _x -= q.x();
    _y -= q.y();
...
template <typename T>
min::quat<T> min::quat<T>::from_x_axis(const T x, const min::vec3<T> &v1)
{
    quat<T> q;
...
template <typename T>
min::quat<T> min::quat<T>::conjugate() const
{
    const T w = _w;
    const T x = -_x;
    const T y = -_y;
...
template <typename T>
T min::quat<T>::dot(const min::quat<T> &q) const
{
    return _w * q.w() + _x * q.x() + _y * q.y() + _z * q.z();
}

template <typename T>
min::quat<T> min::quat<T>::inverse() const
{
    return conjugate();
}
...
template <typename T>
min::quat<T> min::quat<T>::lerp(const min::quat<T> &v0, const min::quat<T> &v1, const T interpolation)
{
...
template <typename T>
min::quat<T> min::quat<T>::interpolate(const min::quat<T> &v0, const min::quat<T> &v1, const T t)
{
    return slerp(v0, v1, t);
}

template <typename T>
T min::quat<T>::magnitude() const
{
    return sqrt(_w * _w + _x * _x + _y * _y + _z * _z);
}

template <typename T>
min::quat<T> &min::quat<T>::normalize()
{
...
template <typename T>
min::quat<T> min::quat<T>::slerp(const min::quat<T> &v0, const min::quat<T> &v1, const T t)
{
    quat<T> v2 = v1;
...
template <typename T>
min::vec3<T> min::quat<T>::transform(const min::vec3<T> &p) const
{
...
template <typename T>
min::quat<T> min::quat<T>::operator*(const min::quat<T> &b) const
{
...
template <typename T>
min::quat<T> &min::quat<T>::operator*=(const min::quat<T> &b)
{
    const T w = b.w() * _w - b.x() * _x - b.y() * _y - b.z() * _z;
    const T x = b.w() * _x + b.x() * _w - b.y() * _z + b.z() * _y;
    const T y = b.w() * _y + b.x() * _z + b.y() * _w - b.z() * _x;
...
template <typename T>
T min::quat<T>::w() const
{
    return _w;
}

template <typename T>
T min::quat<T>::x() const
{
    return _x;
}

template <typename T>
void min::quat<T>::x(const T x)
{
    _x = x;
}

template <typename T>
T min::quat<T>::y() const
{
    return _y;
}

template <typename T>
void min::quat<T>::y(const T y)
{
    _y = y;
}

template <typename T>
T min::quat<T>::z() const
{
    return _z;
}

template <typename T>
void min::quat<T>::z(const T z)
{
    _z = z;
}
