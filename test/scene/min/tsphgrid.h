...
bool test_sphere_grid()
{
    ...
    // vec2 grid
    {
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec2 optimum scale");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec2 point_inside hit 1");
        }
        ...
        // Test get radius; r = sqrt(x^2 + y^2) = 7.071
        r = node->get_cell().get_radius();
        out = out && compare(7.071, r, 1E-4);
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec2 get_cell radius");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec2 point_inside hit 2");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec2 point_inside hit 3");
        }
    ...
    // vec3 grid
    {
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec3 optimum scale");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec3 point_inside hit 1");
        }
        ...
        // Check radius r = sqrt(x^2 + y^2 + z^2) = 8.6602
        r = node->get_cell().get_radius();
        out = out && compare(8.6602, r, 1E-4);
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec3 get_cell radius");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec3 point_inside hit 2");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec3 point_inside hit 3");
        }
    ...
    // vec4 grid
    {
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 optimum scale");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 point_inside hit 1");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 get_cell 1 get_center");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 get_cell 1 cell_extents");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 get_cell radius");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 point_inside hit 2");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 get_cell 2 get_center");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 point_inside hit 3");
        }
        ...
        if (!out)
        {
            throw std::runtime_error("Failed sphere grid vec4 get_cell 3 get_center");
        }
